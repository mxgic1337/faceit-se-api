...
import {findUserProfile, getMatchV2, getPlayerOngoingMatchId} from "../util/user_util";
...
liveRoute.get('/:playerName', (req, res) => {
    findUserProfile(req.params.playerName).then(player => {
        ...
        getPlayerOngoingMatchId(player.id).then(matchId => {
            if (!matchId) {
                res.send(`${req.params.playerName} nie rozgrywa żadnego meczu.`)
                return;
            }

            getMatchV2(matchId).then(match => {
                if (req.query.m === "") {
                ...
                let playerTeam = 1
                if (match.teams.faction1.roster.filter(player1 => player1.id === player.id).length === 0) {
                    playerTeam = 2
                }
                ...
                format = format
                    ...
                    .replace('$team2elo', String(Math.round(team2Elo/match.teams.faction2.roster.length)))
                    .replace('$team2result', match.results ? String(match.results[0].factions.faction2.score) : "0")
                    .replace('$team2', match.teams.faction2.name)
                    .replace('$matchroom', `https://www.faceit.com/pl/cs2/room/${matchId}`)
                res.send(format)
